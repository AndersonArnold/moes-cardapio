...
export default function Home() {
  ...
  // Cart Edit State
  const [cartEditingItemId, setCartEditingItemId] = useState<string | null>(null);
  const [cartEditQuantity, setCartEditQuantity] = useState(1);
  const [cartEditObservation, setCartEditObservation] = useState("");
          ...
          <div className="flex-1 overflow-y-auto w-full">
            ...
            <div className="p-6 space-y-4">
              ...
              {cartItems.length === 0 ? (
                ...
                cartItems.map((item) => (
                  <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-md relative hover:shadow-lg transition-all">
                    {cartEditingItemId === item.id ? (
                      <div className="space-y-3">
                        <div className="flex justify-between items-center mb-2">
                           <h4 className="font-bold text-sm text-zinc-900 leading-tight pr-14">{item.name}</h4>
                           <span className="font-bold text-orange-600 text-sm whitespace-nowrap">
                             R$ {(item.price * cartEditQuantity).toFixed(2).replace('.', ',')}
                           </span>
                        </div>
                        <div className="flex items-center gap-3 bg-gray-100 rounded-full px-2 py-1 w-fit">
                          <button
                        ...
                        <textarea
                          value={cartEditObservation}
                          onChange={(e) => setCartEditObservation(e.target.value)}
                          placeholder="Observações (opcional)"
                          className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm text-zinc-800 focus:outline-none focus:ring-2 focus:ring-orange-500/50 resize-none h-20"
                        />
                        <div className="flex gap-2">
                            <button
                                onClick={() => setCartEditingItemId(null)}
                                className="flex-1 bg-zinc-200 text-zinc-700 font-bold py-2 rounded-xl hover:bg-zinc-300 transition-colors text-sm"
                            >
                                Cancelar
                            </button>
                            <button
                                onClick={() => {
                                    updateItem(item.id, cartEditQuantity, cartEditObservation);
                                    setCartEditingItemId(null);
                                }}
                                className="flex-1 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold py-2 rounded-xl hover:from-orange-600 hover:to-orange-700 transition-all text-sm shadow-sm"
                            >
                                Salvar
                            </button>
                        </div>
                      ...
                      <>
                        <div className="absolute top-4 right-4 flex gap-3">
                          <button
                            onClick={() => {
                                setCartEditingItemId(item.id);
                                setCartEditQuantity(item.quantity);
                                setCartEditObservation(item.observation || "");
                            }}
                            className="text-zinc-400 hover:text-orange-500 transition-colors flex items-center justify-center w-5 h-5 bg-gray-50 rounded-full hover:bg-orange-50"
                            title="Editar"
                          >
                          ...
                          <button
                            ...
                            className="text-zinc-400 hover:text-red-500 transition-colors flex items-center justify-center w-5 h-5 bg-gray-50 rounded-full hover:bg-red-50"
                            title="Remover"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
                        ...
                        {item.observation && (
                          <p className="text-zinc-500 text-xs mt-2 italic bg-gray-100 p-2 rounded-lg border border-gray-200">
